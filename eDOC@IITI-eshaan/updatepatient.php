    ...
    if(empty($_POST['p_email'])||empty($_POST['p_password'])||empty($_POST['FN'])||empty($_POST['LN'])||empty($_POST['CN'])||empty($_POST['rollnum'])||empty($_POST['bg'])||empty($_POST['Gender'])||empty($_POST['roomnum'])){
        ...
        $db=mysqli_select_db($link,"edoc");
            ...
            if(mysqli_query($link, $sql)){
                ...
                $cid = $row2[0] ?? false;
                $sql1 = "UPDATE p_profile SET bg_id='$cid', first_name='$firstname', last_name='$lastname', email='$email', contact_number='$cn', roll_number='$image', gender='$gender', room_number='$description' WHERE patient_id='$doctorid'";     
