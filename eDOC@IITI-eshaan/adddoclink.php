    ...
    if(empty($_POST['d_email'])||empty($_POST['d_password'])||empty($_POST['FN'])||empty($_POST['LN'])||empty($_POST['CN'])||empty($_POST['img'])||empty($_POST['Category'])||empty($_POST['Gender'])||empty($_POST['Description'])){
        ...
        $adminid=$_SESSION['admin_id'];
            ...
            if(mysqli_query($link, $sql)){
                ...
                $cid = $row2[0] ?? false;
                $sql1 = "INSERT INTO d_profile (doctor_id, category_id, first_name, last_name, email, contact_number, image_, gender, description_ ) VALUES ('$did', '$cid', '$firstname', '$lastname', '$email', '$cn', '$image', '$gender', '$description')";    
